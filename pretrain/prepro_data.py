...
seq_home = '../../tracking_benchmark/dataset/'
...
for i,seq in enumerate(seq_list):
    ...
    if seq == 'vot2014/ball':
        img_list.pop()  #delete the last element, because there are 603 images, but gt has 602

    gt = np.loadtxt(seq_home+seq+'/groundtruth.txt',delimiter=',') #maybe need fix
    print(len(img_list))
    print(len(gt))
    ...
    if gt.shape[1]==8: # shape[0] is rows, shape[1] is columns
        ...
        gt = np.concatenate((x_min, y_min, x_max-x_min, y_max-y_min),axis=1) # the seqname is stored in the key of the dictionary, the seq!
