...
class ILSVRC2015RegionDataset(data.Dataset):
    def __init__(self, img_dir, start_frame, end_frame, gt, im_width, im_height,opts):

        self.gt = gt
        self.img_dir = img_dir
        self.start_frame = start_frame
        self.end_frame = end_frame
        self.seq_len = end_frame - start_frame + 1
        self.image_size = (im_width, im_height) # origin_image_size
        self.img_list0 = range(start_frame-1,end_frame,1)
        self.img_list = np.array([os.path.join(img_dir, '%06d.JPEG'%(img)) for img in self.img_list0])
        ...
        self.index = np.random.permutation(self.seq_len)
        self.pointer = 0
        
        # image = Image.open(self.img_list[0]).convert('RGB')
        self.pos_generator = SampleGenerator('gaussian', self.image_size, 0.1, 1.2, 1.1, True)
        self.neg_generator = SampleGenerator('uniform', self.image_size, 1, 1.2, 1.1, True)

        assert len(self.gt) == len(self.img_list), "Length does not match"
    def __iter__(self):
        return self

    def __next__(self):
        next_pointer = min(self.pointer + self.batch_frames, self.seq_len)
        idx = self.index[self.pointer:next_pointer] # idx from 0 while 
        if len(idx) < self.batch_frames:              # start_frame from 1
            self.index = np.random.permutation(self.seq_len)
