...
class MDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        #     ('conv3_attconv1', nn.Sequential(nn.BatchNorm2d(512),
        #         nn.ReLU(),
        #         nn.Conv2d(512, 256, kernel_size=1, stride=1))),
        #     ('conv3_attconv2', nn.Sequential(nn.BatchNorm2d(256),
        #         nn.ReLU(),
        #         nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1))),
        #     ('conv3_attconv3', nn.Sequential(nn.BatchNorm2d(256),
        #         nn.ReLU(),
        #         nn.Conv2d(256, 512, kernel_size=1, stride=1))),
        #     ('conv3_residual', nn.Sequential(nn.Conv2d(512, 512, kernel_size=1, stride=1))),
        #     ('conv3_mask', nn.Sequential(nn.Conv2d(512, 1, kernel_size=1, stride=1),
        #         nn.Softmax2d())),
        #     ('conv3_finetune', nn.Sequential(nn.Conv2d(512, 256, kernel_size=1, stride=1),
        #         nn.BatchNorm2d(256),
        #         nn.ReLU())),
        #     ('conv3_finetune2',nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
        #         nn.BatchNorm2d(256),
        #         nn.ReLU())),
        #     ('conv3_finetune3',nn.Sequential(nn.Conv2d(256, 512, kernel_size=1, stride=1),
        #         nn.BatchNorm2d(512),
        #         nn.ReLU()))
        #     ])) ## nn.Sigmoid() softmax2d
        self.attmask = nn.Sequential(OrderedDict([
            ('conv3_theta', nn.Sequential(nn.Conv2d(512, 256, kernel_size=1, stride=1))),
            ('conv3_phi', nn.Sequential(nn.Conv2d(512, 256, kernel_size=1, stride=1))),
            ('conv3_g', nn.Sequential(nn.Conv2d(512, 256, kernel_size=1, stride=1))),
            ('conv3_out', nn.Sequential(nn.Conv2d(256, 512, kernel_size=1, stride=1)))
            ]))
        ...
        for name, module in self.layers.named_children():# {conv1, conv2,...,}迭代器
            ...
            if run:
                if name == 'conv1':
                    ...
                    # spatial-process
                    # mask_x_rs = mask_x
                    # mask_x = self.attmask[0](mask_x) # 1st route
                    # mask_x = self.attmask[1](mask_x)
                    # mask_x = self.attmask[2](mask_x)
                    # mask_x_rs = self.attmask[3](mask_x_rs) # 2nd route
                    # mask_x = mask_x + mask_x_rs # add
                    # mask_x = self.attmask[5](mask_x) # finetune
                    # mask_x = self.attmask[6](mask_x)
                    # mask_x = self.attmask[7](mask_x)
                    # mask_x = self.attmask[4](mask_x) # mask
                    batch_size = mask_x.size(0)
                    mask_x_theta = self.attmask[0](mask_x)
                    mask_x_phi = self.attmask[1](mask_x)
                    mask_x_g = self.attmask[2](mask_x)
                    mask_x_theta = mask_x_theta.view(batch_size, 256, 9).permute(0, 2, 1)
                    mask_x_phi = mask_x_phi.view(batch_size, 256, 9)
                    mask_x_theta = torch.bmm(mask_x_theta, mask_x_phi)
                    mask_x_theta = F.softmax(mask_x_theta, -1) #??
                    mask_x_g = mask_x_g.view(batch_size, 256, 9).permute(0, 2, 1)
                    mask_x_theta = torch.bmm(mask_x_theta, mask_x_g)
                    mask_x_theta = mask_x_theta.permute(0, 2, 1).contiguous()
                    mask_x_theta = mask_x_theta.view(batch_size, 256, 3, 3)
