...
class MDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        self.attmask = nn.Sequential(OrderedDict([
            ('conv3_theta', nn.Sequential(nn.Conv2d(256, 128, kernel_size=1, stride=1))),
            ('conv3_phi', nn.Sequential(nn.Conv2d(256, 128, kernel_size=1, stride=1))),
            ('conv3_g', nn.Sequential(nn.Conv2d(256, 128, kernel_size=1, stride=1))),
            ('conv3_out', nn.Sequential(nn.Conv2d(128, 256, kernel_size=1, stride=1),
                nn.BatchNorm2d(256)))]))
        ...
        for name, module in self.layers.named_children():# {conv1, conv2,...,}迭代器
            ...
            if run:
                if name == 'conv1':
                    ...
                    mask_x_theta = mask_x_theta.view(batch_size, 128, 25).permute(0, 2, 1)
                    mask_x_phi = mask_x_phi.view(batch_size, 128, 25)
                    mask_x_theta = torch.bmm(mask_x_theta, mask_x_phi)
                    mask_x_theta = F.softmax(mask_x_theta, -1) #??
                    mask_x_g = mask_x_g.view(batch_size, 128, 25).permute(0, 2, 1)
                    mask_x_theta = torch.bmm(mask_x_theta, mask_x_g)
                    mask_x_theta = mask_x_theta.permute(0, 2, 1).contiguous()
                    mask_x_theta = mask_x_theta.view(batch_size, 128, 5, 5)
